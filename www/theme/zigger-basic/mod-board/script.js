        ...
        $ele.add_btn.on('click', function(e) {
            ...
            zigconfirm('현재 작성된 글을 임시저장 하시겠습니까?', function(result) {
                if (result) {
                    ...
                    // temporary form으로 변조
                    $ele.form.attr({
                        'ajax-action' : MOD_BOARD_DIR + '/controller/write/write-temporary-submit?rewritetype=submit',
                    ...
                    // 원본 form 으로 다시 돌려 놓음
                    $ele.form.attr({
                        'ajax-action' : MOD_BOARD_DIR + '/controller/write/write-submit?rewritetype=submit',
                        'id' : 'board-writeForm', 
