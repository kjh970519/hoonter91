...
class Info_submit {
    ...
    public function init()
    {
        ...
        $req = Method::request('post', 'mode, email, current_pwd, pwd, pwd2, name, gender, phone_chg, phone, phone_code, telephone, address1, address2, address3, email_chg_cc');
    ...
    private function get_mdf()
    {
        ...
        // 기존 패스워드가 올바른지 검사
        if (!$req['current_pwd']) Valid::error('current_pwd', '기존 비밀번호를 입력해주세요.');
        
        $sql->query(
            "
            select *
            from {$sql->table("member")}
            where `mb_idx`=:col1 and `mb_id`=:col2 and `mb_pwd`={$sql->set_password($req['current_pwd'])} and
            `mb_dregdate` is null
            ",
            array(
                MB_IDX, $MB['id']
        ...
        if ($sql->getcount() < 1) Valid::error('current_pwd', '기존 비밀번호가 올바르지 않습니다.');
    ...
    private function get_lv()
    {
        ...
        // 비밀번호 입력 되었는지 검사
        if (!$req['current_pwd']) Valid::error('current_pwd', '탈퇴를 위해 비밀번호를 입력하세요.');
        ...
        $sql->query(
            ...
            select *
            from {$sql->table("member")}
            where `mb_idx`=:col1 and `mb_id`=:col2 and `mb_pwd`={$sql->set_password($req['current_pwd'])} and `mb_dregdate` is null
            ",
            array(
                MB_IDX, $MB['id']
            )
        ...
        if ($sql->getcount() < 1) Valid::error('current_pwd', '비밀번호가 올바르지 않습니다.');
