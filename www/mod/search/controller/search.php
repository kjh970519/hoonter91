...
class Search extends \Controller\Make_Controller {
    ...
    public function func()
    {
        ...
        function get_module_contents($keyword, $data_key)
        {
            $sql = new Pdosql();
            ...
            {
                $or = ($key > 0) ? ' and ' : '';
                $where .= $or.'`html` like :col'.($key + 1).' and `data_key`=:contents_id';
                $cols[] = '%'.addslashes($value).'%';
            }
            
            $cols['contents_id'] = $data_key;
        ...
        function get_module_board($keyword, $board, $limit)
        {
            ...
            foreach (exp_keywords($keyword) as $key => $value) {
                $or = ($key > 0) ? ' and ' : '';
                $where[0] .= $or.' `subject` like :col'.($key + 1);
                $where[1] .= $or.' `article` like :col'.($key + 1);
                $where[2] .= $or.' `writer` like :col'.($key + 1);
                $cols[] = '%'.addslashes($value).'%';
            }
    ...
    public function make()
    {
        ...
        $keyword_decode = trim(urldecode($req['keyword']));
        ...
        if ($sql->getcount() > 0) {
            do {
                ...
                if ($opt_exp[0] == 'board') $mod_arr[0]['href'] = $mod_arr['href'].'?keyword='.$keyword_decode;

                // 모듈별 Database 처리
                switch ($opt_exp[0]) {
                    case 'contents' :
                        $mod_arr['data'] = get_module_contents($keyword_decode, $opt_exp[1]);
                        break;

                    case 'board' :
                        $mod_arr['data'] = get_module_board($keyword_decode, $opt_exp[1], $opt_exp[2]);
                        break;
                }
