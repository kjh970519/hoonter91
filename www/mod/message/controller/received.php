...
class Received extends \Controller\Make_Controller {
    ...
    public function make()
    {
        ...
        $keyword = (!empty($where) && !empty(trim($req['keyword']))) ? $req['keyword'] : '';
        ...
        if (!empty($where) && !empty($keyword)) {
            ...
            foreach (exp_keywords($keyword_decode) as $key => $value) {
                $searchby[] = '`'.$where.'` like :searchby_col'.$key;
                $cols['searchby_col'.$key] = '%'.$value.'%';
            }
