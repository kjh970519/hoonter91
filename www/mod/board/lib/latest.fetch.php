...
class Latest_fetch extends \Controller\Make_Controller {
    ...
    public function latest_func()
    {
        ...
        function print_article($list)
        {
            ...
            $html = Func::deHtmlspecialchars($list['article']);
            $html = str_replace('&nbsp;', ' ', $html);
            $html = preg_replace('/\s+/', ' ', $html);
            $html = Func::strcut(strip_tags(Func::deHtmlspecialchars($html)), 0, $FETCH_CONF['article']);
            return $html;
        }
        ...
        function thumbnail($list)
        {
            ...
            // file1에서 썸네일 추출
                ...
                $fileinfo = Func::get_fileinfo($list['file1']);
                if ($fileinfo) {
                    $tmb = ($fileinfo['storage'] == 'Y') ? $fileinfo['replink'] : PH_DOMAIN.PH_DATA_DIR.$fileinfo['filepath'].'/thumb/'.$list['file1'];
                }
            }
            // 본문에서 첫 번째 이미지 추출
            else if ($list['article']) {
                preg_match('/<img[^>]+src=["\']([^"\']+)["\']/', $list['article'], $matches);
                if (!empty($matches[1])) {
                    $img_src = $matches[1];
                    // 상대 경로인 경우 도메인 추가
