...
class Result extends \Controller\Make_Controller {
    ...
    {
        global $PARAM, $sortby, $searchby, $searchby_cols, $orderby;
        ...
        if ($list_cnt > 0) {
            do {
                ...
                $sql2->query(
                    ...
                    where `cfg_type` = :col1
                    ", array(
                        'mod:board:config:'.$arr['cfg_value']
                    )
                );
    ...
    public function form($idx)
    {
        ...
        $form->set('type', 'html');
        $form->set('action', PH_MANAGE_DIR.'/mod/'.MOD_BOARD.'/result/result-clone-submit');
        $form->run();
    }
    ...
    public function init()
    {
        ...
        foreach ($arr as $key => $value) {
            ...
            $sql->query(
                ...
                (:col1, :col2, :col3, now())
                ", array(
                    'mod:board:config:'.$clone_id, $key, $value
                )
            );
        }
...
class Regist extends \Controller\Make_Controller {
    ...
    public function form()
    {
        ...
        $form->set('type', 'html');
        $form->set('action', PH_MANAGE_DIR.'/mod/'.MOD_BOARD.'/result/regist-submit');
        $form->run();
    }
...
class Modify extends \Controller\Make_Controller {
    ...
    public function form()
    {
        ...
        $form->set('type', 'html');
        $form->set('action', PH_MANAGE_DIR.'/mod/'.MOD_BOARD.'/result/modify-submit');
        $form->run();
    }
...
class Modify_submit {
    ...
    public function get_modify()
    {
        ...
        foreach ($data as $key => $value) {
            $sql->query(
                ...
                select *
                from {$sql->table("config")}
                where `cfg_type` = :col1 and `cfg_key` = :col2
                ", array(
            ...
            $sql->query(
                ...
                set
                `cfg_value` = :col1
                where `cfg_type` = :col2 and `cfg_key` = :col3
                ", array(
                    $value, 'mod:board:config:'.$req['id'], $key
                )
            );
        }
    ...
    public function get_delete()
    {
        ...
        $sql->query(
            ...
            where `filepath` like concat(:col1, '%')
            ",
            array(
                '/board/'.$board_id.'/'
            )
...
class Board extends \Controller\Make_Controller {
    ...
    public function func()
    {
        ...
        function file_ico($arr)
        {
            ...
            if ($boardconf['ico_file'] == 'Y' && !empty($arr['file1'])) {
                if ($arr['file1'] == 'image') {
                    return '<img src="'.MOD_BOARD_DIR.'/manage.set/picture-ico.png" align="absmiddle" title="이미지파일" alt="이미지파일" />';
                } else {
                    return '<img src="'.MOD_BOARD_DIR.'/manage.set/file-ico.png" align="absmiddle" title="파일" alt="파일" />';
                }
    ...
    public function make()
    {
        ...
        if ($category) {
            $search = 'and board.category=:category';
            $cols['category'] = addslashes($req['category']);
        }
