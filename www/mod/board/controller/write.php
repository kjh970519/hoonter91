...
class Write_submit{
    ...
    public function init()
    {
        ...
        preg_match_all(REGEXP_IMG, Func::htmldecode($req['article']), $match);

        foreach ($match[1] as $key => $value) {
            if (!$req[0]['ufile_tmb_filename']) {
                // local 파일인 경우만 썸네일 추출
                if (isset($value) && strpos($value, PH_DOMAIN) !== false) {
                    $req[0]['ufile_tmb_filename'] = basename($value);
                }
            }
        }
