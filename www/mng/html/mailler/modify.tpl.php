            ...
            <tr>
                ...
                <td>
                    ...
                    <span class="tbl_sment">
                        ...
                        사이트명: <strong>{{site_title}}</strong> / 메일 제목: <strong>{{subject}}</strong> / 메일 본문: <strong>{{article}}</strong> / 이름: <strong>{{name}}</strong> / 이메일: <strong>{{email}}</strong> / 회원ID: <strong>{{id}}</strong> / 회원임시비밀번호: <strong>{{password}}</strong> / 이메일인증URL: <strong>{{check_url}}</strong>
                    </span>
                    <?php } ?>
                    <?php if ($write['system'] == 'N') { ?>
                    <span class="tbl_sment">
                        사용자 정의 템플릿을 관리합니다. 사용자 정의 템플릿은 아래와 같은 치환자를 제공합니다.<br />
                        사이트명: <strong>{{site_title}}</strong> / 이메일: <strong>{{email}}</strong>
                    </span>
                    <?php } ?>
                </td>
            </tr>
