        ...
        foreach ($arr['check'] as $key => $value) {
            ...
            switch ($key) {
                ...
                case 'charset' :
                    ...
                    if (isset($exp_arr[$value]) && self::trim_val($arr['value']) != '' && !self::match($exp_arr[$value], $arr['value'])) self::error($arr['input'], $arr['msg']);
                ...
                case 'chkhtml' :
                    ...
                    if ($value === true) {
                        ...
                        // XSS 방지를 위한 검증
                        // 이벤트 헨들러 차단
                        if (preg_match('/on[a-z]+\s*=\s*["\']?[^"\'>]*/i', $arr['value'])) $pass_insp = false;
                        // 인코딩 우회 방지 (따옴표 엔티티는 허용, \xHH 패턴만 차단)
                        if (preg_match('/\\\\x[0-9a-f]{2}/i', $arr['value'])) $pass_insp = false;
                        // 위험 함수 호출 탐지
                        if (preg_match('/\b(eval|atob|btoa|XMLHttpRequest|fetch|WebSocket)\s*\(/i', $arr['value'])) $pass_insp = false;
                        // 악성 태그 차단
                        if (preg_match('/<(script|iframe|object|embed)/i', $arr['value'])) $pass_insp = false;
                        // 자바스크립트 프로토콜 차단
                        if (preg_match('/javascript:\s*([\'"]?)/i', $arr['value'])) $pass_insp = false;
                        // 악성 Data URI 차단
                        if (preg_match('/data:\s*(text\/html|image\/svg\+xml)/i', $arr['value'])) $pass_insp = false;
