...
class Pdosql {
    ...
    {
        $tblName = preg_replace('/[^a-zA-Z0-9\-_:]/', ' ', $tblName);
        ...
        try {
            ...
            if (is_array($param)) {
                foreach ($param as $key => $value) {
                    ...
                    // $key가 숫자일 경우
                    if (is_numeric($key)) {
                        $i = $key + 1;  // 숫자로 된 key에 맞는 $i 값 계산 (0-based to 1-based)
                        
                        // :col{$i} 가 query에 있을 경우 처리
                        if (strstr($query, ':col'.$i)) {
                            if (is_null($value)) {
                            ...
                            // :col{$i}를 해당 값으로 치환
                            $qryString = preg_replace('/' . preg_quote(':col' . $i, '/') . '/', (is_null($value) || is_numeric($value)) ? $value : "'$value'", $qryString, 1);
                        }

                    // $key가 문자열인 경우
                    } else {
                        $paramKey = ':' . $key;
                        if (strstr($query, $paramKey)) {
            
                            if (is_null($value)) {
                                $value = '';
                                $this->stmt->bindValue($paramKey, null, \PDO::PARAM_NULL);
                            } else {
                                $value = addslashes($value);
                                $this->stmt->bindValue($paramKey, $value, \PDO::PARAM_STR);
                            }
            
                            // 해당 key를 쿼리에서 찾아서 값으로 치환
                            $qryString = preg_replace('/' . preg_quote($paramKey, '/') . '/', (is_null($value) || is_numeric($value)) ? $value : "'$value'", $qryString);
                        }
                    }
